...
Call `memsy_health` to diagnose. Do not retry blindly or fabricate substitute answers. Be explicit that Memsy isn't reachable right now — suggest the user verify `MEMSY_API_KEY` is set in the MCP env block (shell env is not inherited by the host's MCP process).
