...
- Opt-in **session-start auto-context** (`MEMSY_SESSION_AUTOCONTEXT=on`) that surfaces recent memories via the JSON envelope Codex's SessionStart hook requires. The SessionStart logic runs in `session_start.py` behind a thin `session-start.sh` shim, so it is immune to the macOS-default bash 3.2 heredoc-in-command-substitution parser bug that would otherwise silently disable every session-start-gated feature.
