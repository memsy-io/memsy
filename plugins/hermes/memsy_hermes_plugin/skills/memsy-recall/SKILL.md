...
metadata:
  hermes:
    tags: [memory, recall, memsy]
    category: memory
    requires_toolsets: [memsy]
---
...
Top 3–5 most relevant first. If memories are clearly unrelated to the query, surface only what's relevant. Do **not** invent fields — call `memsy_list_memories` if the user needs to browse without a specific query.
...
Call `memsy_health` to diagnose. Do not retry blindly or fabricate substitute answers. Be explicit that Memsy isn't reachable — suggest the user verify `MEMSY_API_KEY` is set and that `/reload-mcp` has been run after any config changes.
