...
<Steps>
  <Step title="Clone and run the installer">
    ...
    The installer copies the Memsy memory provider to `~/.hermes/plugins/memory/memsy/` and sets `memory.provider: memsy` in `~/.hermes/config.yaml`.
  </Step>
...
```bash
ls ~/.hermes/plugins/memory/memsy/
```
