...
† Toggled by env vars read in the **SessionStart hook** (set them with e.g. `export MEMSY_SESSION_AUTOCONTEXT=on` before launching Codex). They work provided your Codex build forwards the launching shell's environment to hook commands — the [hooks docs](https://developers.openai.com/codex/hooks) don't document shell-env inheritance, so treat it as assumed-pending-confirmation. Unlike these modes, the **API key** must be in `~/.memsy/config.json` / `config.toml`, never a shell `export` (see Set your API key above).
