...
<Steps>
  ...
  <Step title="Set your API key">
    ...
    Codex does not pass your shell environment to MCP servers, so a bare `export MEMSY_API_KEY=...` may not reach the server. If you'd rather configure it by hand, add it to the MCP env block in `~/.codex/config.toml` instead:
