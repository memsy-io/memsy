---
title: Searching Memory
description: Natural-language queries, thresholds, scoping, and score interpretation.
---

`search()` takes a natural-language query and returns ranked memories.

## Basic usage

<Tabs>
  <Tab title="Python">
    ```python
    results = client.search("what does the user prefer?")

    for r in results.results:
        print(r.score, r.content)
    ```
  </Tab>
  <Tab title="Node">
    ```ts
    const results = await client.search('what does the user prefer?');

    for (const r of results.results) {
      console.log(r.score, r.content);
    }
    ```
  </Tab>
  <Tab title="cURL">
    ```bash
    curl -X POST "$MEMSY_BASE_URL/search" \
      -H "Authorization: Bearer $MEMSY_API_KEY" \
      -H "Content-Type: application/json" \
      -d '{"query": "what does the user prefer?"}'
    ```
  </Tab>
</Tabs>

## Parameters

<Tabs>
  <Tab title="Python">
    ```python
    client.search(
        query,                              # required, natural language
        *,
        actor_id=None,                      # optional — filter to one actor
        session_id=None,                    # optional — only this conversation
        exclude_session_id=None,            # optional — every conversation but this one
        limit=10,                           # max results (default 10)
        threshold=0.0,                      # minimum similarity (default 0.0)
        include_source_events=False,        # attach source events to results
    )
    ```
  </Tab>
  <Tab title="Node">
    ```ts
    client.search(query, {
      actorId,                  // optional — filter to one actor
      sessionId,                // optional — only this conversation
      excludeSessionId,         // optional — every conversation but this one
      limit: 10,                // max results (default 10)
      threshold: 0.0,           // minimum similarity (default 0.0)
      includeSourceEvents: false,
    });
    ```
  </Tab>
  <Tab title="cURL">
    ```json
    {
      "query": "natural language",
      "actor_id": "user_42",
      "session_id": "conv_7",
      "limit": 10,
      "threshold": 0.0,
      "include_source_events": false
    }
    ```
  </Tab>
</Tabs>

### `query`

Plain English. You don't need to phrase it like a keyword search — "things the user mentioned about their dog" works.

### `actor_id` / `actorId`

Pass it to scope the search to one user/agent. Omit it to search across every actor in the organization. In most user-facing flows you want to pass the current actor.

### `session_id` / `sessionId`

Scope the search to a single conversation. Pass the same conversation id you used when ingesting those events — `session_id` is required at ingest, so you already have it.

> **Search accepts a narrower set of ids than ingest does.** Searchable ids may contain only letters, digits and `_ - : . @ /`, up to 256 characters. Ingest checks the length but **not** the character set — so an id containing a space, `#`, `+`, `~`, an emoji or any non-ASCII character stores fine and returns a `422` the first time you search it. This bites whenever a conversation id comes from something a human wrote: a chat title, a thread name, a channel description. If you don't control the id's shape, slugify it before ingest rather than finding out at search time.

### `exclude_session_id` / `excludeSessionId`

The mirror image: return everything **except** one conversation. Useful for "what have we discussed before?" when you want previous conversations and not the one you're already in.

Three ways to ask, then:

| You want | Send |
|---|---|
| only this conversation | `session_id` |
| every conversation but this one | `exclude_session_id` |
| this one **and** the previous ones | neither — the unchanged default |

Three things to know:

**One conversation at a time.** Both fields take a single id, not a list — unlike `role_ids` / `team_ids` alongside them. Passing a list returns a `422`. There is no way to ask for several named conversations in one search; run one search per conversation, or use `exclude_session_id` if what you want is "all but one".

**Memories that belong to no conversation always come through.** Promoted role, team and org knowledge is stored without a conversation on purpose — it's general, not from one chat. Both filters let it through, so scoping a search never hides what Memsy knows about the actor generally.

**The two are mutually exclusive.** Sending both *non-empty* returns a `422`. They don't cancel out into "search everything" — for that, send neither.

**An empty string is not the same as sending both.** The server trims each id and treats an empty result as unset, and it does that *before* the mutual-exclusion check. So `session_id: ""` together with `exclude_session_id` does not error — it runs as a plain exclude search. Something like `session_id=current or ""` therefore scopes differently from what you meant, with a `200` and no warning. Pass `None` / `undefined` when you have no conversation, never `""`.

### `limit`

How many results to return. Memsy may return fewer if not enough pass the threshold. Valid range: 1–100.

### `threshold`

A floor on the similarity score — results scoring below it are dropped. **The SDK default is `0.0` (no filter)**, so out of the box you get the top `limit` results ranked by relevance.

Raise the threshold to cut noise. The right value depends on tier:

- **Free (no reranking)**: raw retrieval scores cluster in `0.0`–`0.1` even for strong matches. Stay near `0.0`–`0.05`.
- **Pro and above (with Cohere reranking)**: scores are normalised to `0.0`–`1.0`. A threshold around `0.3`–`0.5` becomes a meaningful "good match" floor; `0.8`+ returns only near-exact matches.

Scores are not comparable across queries — `0.6` for query A is not the same thing as `0.6` for query B. Treat `threshold` as a relative tuning knob, not an absolute quality bar.

### `include_source_events` / `includeSourceEvents`

When `true`, each result additionally includes the source events the memory was extracted from. Useful for showing provenance in a UI.

## The result shape

<Tabs>
  <Tab title="Python">
    ```python
    results.results  # list[SearchResult]

    r.id                # stable memory ID
    r.content           # natural-language memory, e.g. "User prefers dark mode"
    r.score             # float; higher = closer match
    r.metadata          # dict | None — server-attached fields (type, kind, …)
    r.strength          # float — reinforcement strength, bounded 0.0–5.0
    r.session_id        # str | None — conversation it came from; None if it belongs to none
    r.source_metadata   # list[dict] — user metadata from the originating event(s)
    ```
  </Tab>
  <Tab title="Node">
    ```ts
    results.results;  // SearchResult[]

    r.id;              // stable memory ID
    r.content;         // natural-language memory
    r.score;           // number; higher = closer match
    r.metadata;        // Record<string, unknown> | null
    r.sessionId;       // string | null — conversation it came from; null if it belongs to none
    r.sourceMetadata;  // SourceMetadata[] | null — user metadata from source events
    ```
  </Tab>
  <Tab title="cURL">
    ```json
    {
      "results": [
        {
          "id": "01JK...",
          "content": "User prefers dark mode across apps.",
          "score": 0.84,
          "metadata": {
            "type": "preference",
            "strength": 1.5,
            "session_id": "conv_7",
            "source_metadata": [
              { "event_id": "01JK...", "metadata": { "plan": "pro" } }
            ]
          }
        }
      ]
    }
    ```
  </Tab>
</Tabs>

`strength` is a reinforcement signal bounded `0.0`–`5.0` by the platform's policy ceiling; it rises by a small amount each time a memory is returned by `search()` and decays slowly when unused. **Not** a probability — don't normalise it into the `[0, 1]` range.

`session_id` is the conversation the memory came from. It is **absent** when the memory belongs to no conversation — which is itself the signal that it's general knowledge rather than something said in one chat. That's what makes searching across conversations readable: send neither filter and you get everything, with each result saying where it came from, so you can tell "you mentioned this earlier today" from "you said this last week".

`source_metadata` carries the JSON metadata you passed at ingest, attached to the originating event(s) the memory was derived from. Up to 5 entries; each entry is `{event_id, metadata: {...}}` for valid JSON-object payloads or `{event_id, raw: "..."}` otherwise. Use it for tagging, filtering, or correlating back to your application's own records.

## Reading usage and rate-limit info

Every response carries observability metadata:

<Tabs>
  <Tab title="Python">
    ```python
    results = client.search("preferences", actor_id="user_42")

    if results.usage:
        print(f"Plan: {results.usage.plan}")
        print(f"Search queries: {results.usage.search_queries}/{results.usage.search_queries_limit}")

    if results.rate_limit:
        print(f"Remaining this window: {results.rate_limit.remaining}")
    ```
  </Tab>
  <Tab title="Node">
    ```ts
    const results = await client.search('preferences', { actorId: 'user_42' });

    if (results.usage) {
      console.log(`Plan: ${results.usage.plan}`);
      console.log(
        `Search queries: ${results.usage.searchQueries}/${results.usage.searchQueriesLimit}`,
      );
    }

    if (results.rateLimit) {
      console.log(`Remaining this window: ${results.rateLimit.remaining}`);
    }
    ```
  </Tab>
  <Tab title="cURL">
    ```bash
    # Read response headers — same data the SDKs parse for you.
    curl -i -X POST "$MEMSY_BASE_URL/search" \
      -H "Authorization: Bearer $MEMSY_API_KEY" \
      -H "Content-Type: application/json" \
      -d '{"query": "preferences", "actor_id": "user_42"}' | grep -E "X-(Usage|Plan|RateLimit)-"
    ```
  </Tab>
</Tabs>

See **[Usage and Rate Limits](/docs/usage-and-rate-limits)** for every field.

## Common patterns

### Personalisation at prompt time

<Tabs>
  <Tab title="Python">
    ```python
    memories = client.search(
        query=user_message,
        actor_id=user_id,
        limit=5,
        threshold=0.4,
    ).results

    system_preamble = "Relevant context:\n" + "\n".join(f"- {m.content}" for m in memories)
    ```
  </Tab>
  <Tab title="Node">
    ```ts
    const { results: memories } = await client.search(userMessage, {
      actorId: userId,
      limit: 5,
      threshold: 0.4,
    });

    const systemPreamble =
      'Relevant context:\n' + memories.map((m) => `- ${m.content}`).join('\n');
    ```
  </Tab>
  <Tab title="cURL">
    ```bash
    # In a real app, this lives in your backend code — cURL example shown for clarity only.
    curl -X POST "$MEMSY_BASE_URL/search" \
      -H "Authorization: Bearer $MEMSY_API_KEY" \
      -H "Content-Type: application/json" \
      -d "{\"query\": \"$USER_MESSAGE\", \"actor_id\": \"$USER_ID\", \"limit\": 5, \"threshold\": 0.4}"
    ```
  </Tab>
</Tabs>

### Cross-actor search (admin tools)

Leave `actor_id`/`actorId` unset and you'll search every actor in the org. Good for analytics dashboards and support tooling; almost never what you want in an end-user-facing agent loop.

### Empty results

`search()` always returns a response object. If nothing passed the threshold, `results.results` is `[]`. Handle that case in your UI.

## Limitations

Search only returns memories that have finished processing. Very recent events may still be in the queue — see **[Async Processing](/docs/async-processing)**.

`session_id` and `exclude_session_id` need a Memsy server that supports conversation-scoped search. Against an older server they are **silently ignored** — unknown fields are dropped, so you get a `200` and an unfiltered result set with no indication the scope wasn't applied.

## Next

- Run the same thing from asyncio (Python) → **[Async client](/docs/async-client)**.
