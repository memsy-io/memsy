...
<Note>
The CRUD operations on this page (creating/listing/updating orgs, roles, and teams) are currently exposed only on the **Python SDK** via `client.orgs`, `client.roles`, `client.teams`. The Node SDK can still **tag events** with `roleId`/`teamId` once the hierarchy exists — see the [last section on this page](#tagging-events-with-roleteam). Org/role/team management from Node is on the roadmap; for now use the Python SDK or call the control-plane HTTP endpoints directly.
</Note>
...
See **[Models](/docs/reference/python/models)** for the full `OrgResource`, `RoleResource`, and `TeamResource` definitions.
...
- **[`MemsyClient`](/docs/reference/python/memsy-client)** — full sub-resource method signatures.
