...
### SessionStart conversation note (always on)

The same hook also records which conversation this window is in, so `memsy_search` can be scoped to it. This part is **not** opt-in and produces no output — it writes a small file under `CLAUDE_PLUGIN_DATA` (falling back to `~/.memsy/`), named for the `claude` process it belongs to:

```json
{"session_id": "3840ad0d-…", "source": "clear", "updated_at": 1789625823}
```

It exists because the MCP server is told the conversation id **once**, when it launches, and then survives `/clear` — which starts a new conversation without restarting it. SessionStart fires on startup, resume, clear and compact, so the note stays correct where the launch value goes stale. The MCP finds it by reading the same process id out of its own environment.

If the note is missing or unreadable, the MCP falls back to the launch value, and failing that sends no conversation filter at all — a scoped search degrades to searching everything rather than guessing.
...
> **Conversation alignment.** Turn sync also tags each event with the conversation id Claude Code passes to its hooks — the same one the MCP resolves — so a `this_conversation` search finds turns captured here. Without that agreement the two would file the same chat under different names and a scoped search would silently miss everything turn sync stored. Older hosts that don't supply a conversation id fall back to a hash of the transcript path: stable per chat, but unknown to the MCP, so those memories stay findable by actor and by semantic match and not by conversation scope.
...
| Memsy tools behave like an old version (e.g. `/memsy list` not actor-scoped) | A global `@memsy-io/mcp` shadows the plugin's `npx` | `npm ls -g @memsy-io/mcp` → if listed, `npm uninstall -g @memsy-io/mcp`, then restart. See [MCP troubleshooting](/docs/mcp#troubleshooting). |
