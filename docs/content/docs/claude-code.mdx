...
> **Identity alignment (important).** Turn sync must tag each event with the same `actor_id` that `memsy_search` reads, or captured memories won't surface in recall. The hook mirrors the MCP's derivation exactly — `MEMSY_ACTOR_ID` if set, otherwise `sha256("<profile>|<git-email>")`. They can diverge in one case: if you select a profile via `active_profile` in `~/.memsy/config.json` (rather than the `MEMSY_PROFILE` env var) **and** don't set `MEMSY_ACTOR_ID`, the hook assumes the `default` profile while the MCP uses your file's active profile. **Fix: set `MEMSY_ACTOR_ID` explicitly** so both sides agree.
...
| `MEMSY_TURN_SYNC` | `on` to auto-capture every user+assistant turn via the `Stop` hook (POSTs directly to `/ingest`). Default: `off`. Set `MEMSY_ACTOR_ID` alongside it if you use a `~/.memsy/config.json` `active_profile` (see Turn sync above). Accepts `on`/`true`/`1`/`yes`/`enabled`. |
...
| Turn-synced memories not surfacing in recall | `actor_id` mismatch — hook wrote under a different actor than search reads (config-file `active_profile` without `MEMSY_ACTOR_ID`) | `export MEMSY_ACTOR_ID=<stable-id>` so the `Stop` hook and the MCP agree. |
