---
title: MCP Server
description: Drop-in memory infrastructure that learns at the role, team, and org level — so your AI app gets smarter as more agents and humans use it. Works with Claude Code, Cursor, VS Code, Cline, Continue.dev, Zed.
---

The Memsy [MCP](https://modelcontextprotocol.io) server is drop-in memory infrastructure that learns at the **role**, **team**, and **org** level — so your AI app gets smarter as more agents and humans use it. One server, every host.

## What you get

14 tools your AI agent can call:

| Tool | Purpose |
|---|---|
| `memsy_search` | Semantic search across stored memories, optionally scoped to one conversation. |
...
<Steps>
  ...
  <Step title="Verify it's connected">
    <Tabs groupId="mcp-host" persist>
      <Tab title="Claude Code">
        Type `/mcp` — `memsy` should appear with 14 tools.
      </Tab>
      <Tab title="Cursor">
...
#### Scoping a search to a conversation

`memsy_search` takes a `scope`, which decides which conversations it looks in:

| `scope` | Searches |
|---|---|
| `all` *(default)* | Everything — every past chat **and** all connector-sourced memory. |
| `this_conversation` | Only the current chat, plus general knowledge belonging to no conversation. |
| `everything_except_this_conversation` | Earlier chats and connector memory, leaving out what's already in front of you. |

Say *"only what we said in this chat"* or *"have we discussed this before, other than just now?"* and the agent picks the matching scope.

> **`this_conversation` is narrower than it sounds.** Memories from Google Drive, Slack, GitHub, Notion, S3 and OneDrive each carry their *own* conversation id — the folder, channel or repo they came from — so a chat-scoped search excludes all of them, along with every earlier chat. That's why `all` is the default: scoping is opt-in, and switching it on switches connector memory off.

Each result also reports the `session_id` it came from, so an `all` search can be read by origin — `null` means the memory belongs to no conversation, which is how promoted role/team/org knowledge is stored.

If the host can't tell the MCP which conversation is current, a scoped search **degrades to `all`** rather than guessing, and the response says so in its `scope` field — for example `all (requested scope unavailable: this conversation could not be identified)`. Returning too much is visible; returning the wrong conversation's memories would not be.
...
If the search was scoped, check the `scope` field in the response:

- `this_conversation` excludes **all** connector memory (Drive, Slack, GitHub, Notion, S3, OneDrive) and every earlier chat. A `count: 0` here often just means the answer lives outside this chat — re-run with the default `all`.
- `all (requested scope unavailable: …)` means the scope was asked for but dropped, because the host didn't tell the MCP which conversation is current. Those results are unscoped, not empty.
