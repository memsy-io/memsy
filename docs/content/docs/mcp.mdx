...
11 tools your AI agent can call:
...
**Onboarding** (recommended first interaction):
- *"Set up my Memsy defaults."* — agent invokes the `setup-defaults` prompt, lists roles and teams, asks you to pick, and persists the choice to `~/.memsy/config.json`.
- *"Set my default role to senior-eng and team to platform, save it."* — direct path if you already know the IDs.

The agent translates these into the right `memsy_search` / `memsy_ingest` / `memsy_list_memories` / `memsy_set_defaults` calls.
