...
<Tabs>
  ...
  <Tab title="Node">
    ...
    console.log(`${result.total} total memories`);
    for (const m of result.items) {
      console.log(m.memoryId, m.text, m.confidence);
    }
    ```
  </Tab>
</Tabs>
...
<Tabs>
  ...
  <Tab title="Node">
    ...
    while (true) {
      const page = await client.memories.list({ limit: pageSize, offset });
      for (const m of page.items) await process(m);
      offset += pageSize;
...
<Tabs>
  ...
  <Tab title="Node">
    ...
    console.log(memory.text, memory.strength, memory.tags);
    console.log(memory.scope.level, memory.scope.actorId);
    ```
  </Tab>
</Tabs>
...
<Tabs>
  ...
  <Tab title="Node">
    ...
    console.log(`Total: ${stats.totalMemories}, Active: ${stats.activeMemories}`);
    console.log(`Avg confidence: ${stats.avgConfidence.toFixed(2)}`);
    console.log('By type:', stats.byType);
    console.log('By kind:', stats.byKind);
    console.log('Top entities:', stats.topEntities.slice(0, 5));
    ```
  </Tab>
</Tabs>
